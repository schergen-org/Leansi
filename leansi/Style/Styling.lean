...
def bold (s : Style := {}) : Style := { s with isBold := true }
/-- Makes text appear <u>underlined</u> -/
def underline (s : Style := {}) : Style := { s with isUnderline := true }
/-- Makes text appear *italic* -/
def italic (s : Style := {}) : Style := { s with isItalic := true }
/-- Makes text appear dim -/
def dim (s : Style := {}) : Style := { s with isDim := true }
/-- Makes text appear blinking -/
def blink (s : Style := {}) : Style := { s with isBlink := true }
/-- Reverses the colors of the text -/
def reverse (s : Style := {}) : Style := { s with isReverse := true }
/-- Hides the text -/
def hidden (s : Style := {}) : Style := { s with isHidden := true }
/-- Makes the text appear strikethrough -/
def strikethrough (s : Style := {}) : Style := { s with isStrikethrough := true }

/-- Apply an ANSI 16 foreground color code to a style.
The optional base style lets callers update the foreground while keeping other
style choices intact. -/
def fg_ansi_16 (code : Nat) (s : Style := {}) : Style :=
  { s with foreground := some (ColorLevel.ansi16 code) }
...
/-- Apply an ANSI 16 background color code to a style. -/
def bg_ansi_16 (code : Nat) (s : Style := {}) : Style :=
  { s with background := some (ColorLevel.ansi16 code) }
...
def fg_ansi_256 (color : Nat) (s : Style := {}) : Style :=
  { s with foreground := some (ColorLevel.ansi256 color) }

/-- Set background color using ANSI 256 palette indices. -/
def bg_ansi_256 (color : Nat) (s : Style := {}) : Style :=
  { s with background := some (ColorLevel.ansi256 color) }
